...
# 物件
# 人物 障礙物 分數 地板線條 開始結束遊戲文字 最高分數 跑步姿勢 失敗失誤 各種聲音 背景圖
# 事件
# 按下上或是滑鼠左鍵一下跳起來 按下下蹲下 滑鼠點到任意地方開始遊戲  遊戲啟動分數開始 遊戲結束分數停止 
# 超過分數會發出聲 跳起來有聲音 遊戲開始發出聲 撞到障礙物人物停止不動 
#死亡後 分數重新計算
#在死亡時把 腳色放到視窗外 並在按下空白後 先跳到遊戲畫面 過兩秒再回到視窗內
